...
$query = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'DIV' AND `Date1` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `paytype` like 'S' AND `localcode` NOT LIKE '% %'";
...
foreach($rs as $record)
        ...
        if ($status != "delisted" && $count > 0)
            {
            //print "add record";
            $insert_query = "INSERT INTO `stock`.`stock_dividend_ca` (`uid`, `action_id`, `symbol`, `name`, `issue_date`, `update_date`, `effective_date`, `record_date`, `pay_date`, `stock_dividend_type`, `percent`, `dividend_symbol`, `adjustment`, `adjustment_date`, `notes`, `alias_id`, `fb_aliaskey`) VALUES ('','".$actionid."','".$symbol."','".$name."','".$issue_date."','".$update_date."','".$effective_date."','".$record_date."','".$pay_date."','".$stock_dividend_type."','".$percent."','".$dividend_symbol."','".$adjustment."','".$adjustment_date."','".$notes."','".$alias_id."','".$fb_aliaskey."')";
            print $insert_query;
                try{
