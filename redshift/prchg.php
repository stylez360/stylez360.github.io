...
$query = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'PRCHG' AND `Date1` LIKE '$today' AND `cntryofincorp` LIKE 'US' AND `localcode` NOT LIKE '% %'";
...
('', '$a_stock_id', '$a_symbol', '$a_date', '$a_name', '$a_stock_status', '$field3', '$a_cusip', '$a_fb_stockkey', '$a_fb_aliaskey', '$a_sector', '$a_style', '$a_current')";
            ...
            //print "add record";
            $insert_query = "INSERT INTO `stock`.`exchange_change_ca` (`uid`, `action_id`, `symbol`, `name`, `issue_date`, `update_date`, `effective_date`, `old_exchange_code`, `new_exchange_code`, `notes`, `alias_id`, `fb_aliaskey`) VALUES ('', '$actionid', '$symbol', '$name', '$issue_date', '$update_date', '$effective_date', '$field2', '$field3', '$notes', '$alias_id', '$fb_aliaskey');";
            print $insert_query;
                try{
