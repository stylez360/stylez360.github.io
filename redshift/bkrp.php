...
$query = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'BKRP' AND `Date1` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `localcode` NOT LIKE '% %'";
...
foreach($rs as $record)
        ...
        if ($status != "delisted" && $count > 0)
            {
            //print "add record";
            $insert_query = "INSERT INTO `stock`.`bankruptcy_ca` (`uid`, `action_id`, `symbol`, `name`, `issue_date`, `update_date`, `effective_date`, `bankrupcy_type`, `notes`, `alias_id`, `fb_aliaskey`) VALUES ('','".$actionid."','".$symbol."','".$name."','".$issue_date."','".$update_date."','".$effective_date."','".$bankrupcy_type."','".$notes."','".$alias_id."','".$fb_aliaskey."')";
            print $insert_query;
                try{
