...
$query = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'LCC' AND `Date1` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `localcode` NOT LIKE '% %'";
...
('', '$a_stock_id', '$a_symbol', '$a_date', '$a_name', '$a_stock_status', '$a_exchange', '$field5', '$a_fb_stockkey', '$a_fb_aliaskey', '$a_sector', '$a_style', '$a_current');";
            print "insert_alias_sql - ".$insert_alias_query."<br>";
            try{
                    $iaq = $fLink->prepare($insert_alias_query);
                    $iaq->execute();
                    }
                    catch(PDOException $error){
                        // Log any error
            ...
            try{
                    $oau = $fLink->prepare($old_alias_update);
                    $oau->execute();
                    }
                    catch(PDOException $error){
                        // Log any error
            ...
            //print "add record";
            $insert_query = "INSERT INTO `stock`.`symbol_change_ca` (`uid`, `action_id`, `symbol`, `name`, `cusip`, `issue_date`, `update_date`, `effective_date`, `new_symbol`, `notes`, `alias_id`, `fb_aliaskey`) VALUES ('', '$actionid', '$field2', '$name', '$cusip', '$issue_date', '$update_date', '$effective_date', '$field3', '', '$alias_id', '');";
            print $insert_query;
                try{
