...
$query = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'CONSD' AND `Date1` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `localcode` NOT LIKE '% %'";
...
foreach($rs as $record)
        ...
        //$rate2 = $record['Rate2'];
        $cusip =  $record['uscode'];
        $split_ratio = $record['RatioNew'] / $record['RatioOld'];
        $ratio_new = floor($record['RatioNew']);
        $ratio_old = floor($record['RatioOld']);
        $split_terms = $ratio_new." for ".$ratio_old;
        ...
        if ($status != "delisted" && $count > 0)
            {
            //print "add record";
            $insert_query = "INSERT INTO `stock_split_ca` (`uid`, `action_id`, `symbol`, `name`, `cusip`, `issue_date`, `update_date`, `effective_date`, `split_type`, `split_terms`, `split_ratio`, `notes`, `alias_id`, `fb_aliaskey`) VALUES
('', '$actionid', '$symbol', '$name', '$cusip', '$issue_date', '$update_date', '$effective_date', '3000', '$split_terms', '$split_ratio', '', '$alias_id', '');";
            print $insert_query;
                try{
