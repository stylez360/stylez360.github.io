    ...
    case 'R':
        ...
        for ($user_count=1; $user_count <=mysqli_num_rows($user_result); $user_count++)
            ...
            if ($user_row['last_sms'] < time()-900 && $user_row['number'] != "")
                ...
                $sms_url = 'http://api.clickatell.com/http/sendmsg?user=ewomennetwork&password=addison75254&api_id=3388567&MO=1&from=17752374681&to='.$number.'&text='.$message;
            ...
            for ($user_count=1; $user_count <=mysqli_num_rows($user_result); $user_count++)
                ...
                $id = $user_row['id'];
                $email = $user_row['email'];
                if ($user_row['last_email'] < time()-900 && $user_row['email'] != "")
                    {
                    //send email
                    $to = $email;
 				    $subject = " Red Alert!";
				...
				if (mail($to, $subject, $body))
					...
					//update last_mail
                    $email_sent = time();
                    $update_sql = "UPDATE `users` SET `last_email` = '$email_sent' WHERE `id` = '$id';";
                    //print $update_sql;
                    mysqli_query($sysmon_link, $update_sql);
        ...
        $user_sql = "SELECT * FROM `users`;";
            ...
            for ($user_count=1; $user_count <=mysqli_num_rows($user_result); $user_count++)
                ...
                if ($user_row['last_email'] < time()-900 && $user_row['email'] != "")
				    ...
				    $body = 'Stock feed seems to be slow (Yellow).  Current loop is '.$delay.' seconds and counting';
				if (mail($to, $subject, $body))
					{
					//update last_mail
                    $email_sent = time();
