...
$count_sql = "SELECT count(*) as count FROM `symbols_error`;";
...
for ($loop=1; $loop <=$num_of_requests; $loop++)
    {
    $string = "";
    $symbol_sql = "SELECT * FROM `symbols_error` LIMIT $step, $results_per_request;";
    ...
    for ($counter=1; $counter <= mysqli_num_rows($symbol_result); $counter++)
        ...
        $symbol = $symbol_row['symbol'].".".$symbol_row['mid'];
        $string .=  $symbol.",";
        }
    $string = rtrim($string, ",");
    include "e_daily_engine.php";
