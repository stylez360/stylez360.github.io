...
foreach($json as $key => $value)
    ...
    $mTimeStamp = time();
    //explode Symbol to drop the mid and return just the symbol as $nSym (This is e_feed stuff only)
    $nSym = substr($Symbol, 0, strrpos($Symbol, '.'));
    $feed_update = "UPDATE `stock_feed` SET `Outcome` = '$Outcome', `Message` = '$Message', `Date` = '$Date', `Time` = '$Time', `Open` = '$Open', `Close` = '$Close', `High` = '$High', `Low` = '$Low', `Last` = '$Last', `LastSize` = '$LastSize', `Volume` = '$Volume', `PreviousClose` = '$PreviousClose', `PreviousCloseDate` = '$PreviousCloseDate', `ChangeFromPreviousClose` = '$ChangeFromPreviousClose', `PercentChangeFromPreviousClose` = '$PercentChangeFromPreviousClose', `Bid` = '$Bid', `BidSize` = '$BidSize', `BidDate` = '$BidDate', `BidTime` = '$BidTime', `Ask` = '$Ask', `AskSize` = '$AskSize', `AskDate` = '$AskDate', `AskTime` = '$AskTime', `High52Weeks` = '$High52Weeks', `Low52Weeks` = '$Low52Weeks', `TradingHalted` = '$TradingHalted', `CIK` = '$CIK', `Valoren` = '$Valoren', `mTimeStamp` = '$mTimeStamp' WHERE `Symbol` = '$nSym';";
    mysqli_query($data_link, $feed_update);
    //insert into stock_feed_history
    $history_sql = "INSERT INTO `stock_feed_history` (`uid`, `Symbol`, `Last`, `mTimeStamp`) VALUES ('', '$Symbol', '$Last', '$mTimeStamp');";
    //mysqli_query($data_link, $history_sql);
