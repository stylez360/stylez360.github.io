...
$count_sql = "SELECT count(*) as count FROM `feed2`.`stock_feed` WHERE `Outcome` LIKE 'Success';";
...
for ($loop=1; $loop <=$num_of_requests; $loop++)
    ...
    $symbol_sql = "SELECT * FROM `feed2`.`stock_feed` WHERE `Outcome` LIKE 'Success' LIMIT $step, $results_per_request;";
    //print $symbol_sql;
